...
namespace Aperture3D.Math
{
    ...
    public struct Matrix : IEquatable<Matrix>
	{
		...
		public static Vec3 GetTranslation(Matrix m)
		{
            ...
            ref Vec3 cameraUpVector, Vec3? cameraForwardVector, out Matrix result)
		{
			...
			if (num < 0.0001f) {
				...
				Vec3.Multiply (ref vector, (float)(1f / ((float)System.Math.Sqrt ((double)num))), out vector);
			}
		...
		public static Matrix CreateConstrainedBillboard (Vec3 objectPosition, Vec3 cameraPosition,
            Vec3 rotateAxis, Nullable<Vec3> cameraForwardVector, Nullable<Vec3> objectForwardVector)
		{
			Matrix result;
		...
		public static void CreateConstrainedBillboard (ref Vec3 objectPosition, ref Vec3 cameraPosition,
            ref Vec3 rotateAxis, Vec3? cameraForwardVector, Vec3? objectForwardVector, out Matrix result)
		{
			float num;
			Vec3 vector;
			Vec3 vector2;
			...
			if (num2 < 0.0001f) {
				vector2 = cameraForwardVector.HasValue ? -cameraForwardVector.Value : Vec3.Forward;
			} else {
				Vec3.Multiply (ref vector2, (float)(1f / ((float)System.Math.Sqrt ((double)num2))), out vector2);
			}
			Vec3 vector4 = rotateAxis;
			Vec3.Dot (ref rotateAxis, ref vector2, out num);
			if (System.Math.Abs (num) > 0.9982547f) {
				if (objectForwardVector.HasValue) {
					vector = objectForwardVector.Value;
					Vec3.Dot (ref rotateAxis, ref vector, out num);
					if (System.Math.Abs (num) > 0.9982547f) {
						num = ((rotateAxis.X * Vec3.Forward.X) + (rotateAxis.Y * Vec3.Forward.Y)) + (rotateAxis.Z * Vec3.Forward.Z);
						vector = (System.Math.Abs (num) > 0.9982547f) ? Vec3.Right : Vec3.Forward;
					}
				} else {
					num = ((rotateAxis.X * Vec3.Forward.X) + (rotateAxis.Y * Vec3.Forward.Y)) + (rotateAxis.Z * Vec3.Forward.Z);
					vector = (System.Math.Abs (num) > 0.9982547f) ? Vec3.Right : Vec3.Forward;
				}
				Vec3.Cross (ref rotateAxis, ref vector, out vector3);
				vector3.Normalize ();
				Vec3.Cross (ref vector3, ref rotateAxis, out vector);
				vector.Normalize ();
			} else {
				Vec3.Cross (ref rotateAxis, ref vector2, out vector3);
				vector3.Normalize ();
				Vec3.Cross (ref vector3, ref vector4, out vector);
				vector.Normalize ();
			}
			result.M11 = vector3.X;
			result.M12 = vector3.Y;
		...
		{
			var vector = Vec3.Normalize (cameraPosition - cameraTarget);
			var vector2 = Vec3.Normalize (Vec3.Cross (cameraUpVector, vector));
			var vector3 = Vec3.Cross (vector, vector2);
			...
			result.M33 = vector.Z;
			result.M34 = 0f;
			result.M41 = -Vec3.Dot (vector2, cameraPosition);
...
#if PSM
		public static implicit operator Sce.PlayStation.Core.Matrix4 (Matrix x)
		{
			return new Sce.PlayStation.Core.Matrix4 (
		...
		public static implicit operator Matrix (Sce.PlayStation.Core.Matrix4 x)
		{
			return new Matrix (
				x.M11, x.M12, x.M13, x.M14,
				x.M21, x.M22, x.M23, x.M24,
...
#elif PC
        public static implicit operator Microsoft.Xna.Framework.Matrix(Matrix x)
        {
        ...
        public static implicit operator Matrix(Microsoft.Xna.Framework.Matrix x)
        {
            return new Matrix(
                x.M11, x.M12, x.M13, x.M14,
                x.M21, x.M22, x.M23, x.M24,
